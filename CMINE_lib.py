...
def sample_batch(data, arrange=[[0],[1],[2]], batch_size=100, sample_mode='joint', K_neighbor=10, radius=1000):
    ...
    elif sample_mode == 'prod_simple':
        # Only used for MI estimation. sample according to p(x)p(y)
        index_x = np.random.choice(range(N), size=batch_size, replace=False)
        index_y = np.random.choice(range(N), size=batch_size, replace=False)
        batch = np.concatenate((X[index_x],Y[index_y]),axis=1)                 
...
def batch_construction(data, arrange, set_size=100, K_neighbor=2, mode='memoryless', depth=0):   
    ...
    if mode == 'memoryless':
        ...
        train_index = np.arange(set_size)
        ...
        if not arrange[2]:
            prod_train = sample_batch(Train_set, arrange, batch_size=set_size,sample_mode='prod_simple',K_neighbor=K_neighbor)
        else:
        ...
        if not arrange[2]:
            prod_test = sample_batch(Test_set, arrange, batch_size=set_size,sample_mode='prod_simple',K_neighbor=K_neighbor)
        else:
...
def create_dataset(GenModel, Params, Dim=1, N=10000):
    ...
    if GenModel == 'Gaussian_nonZero':
    ...
    elif GenModel == 'Gaussian_Queue':
        (A, sigma_x, sigma_y) = Params
        
        x = Nx = np.random.multivariate_normal(mean=[0]*Dim,
                                     cov=sigma_x**2*np.eye(Dim),
                                     size=N+1)


        y = Ny = np.random.multivariate_normal(mean=[0]*Dim,
                                     cov=sigma_y**2*np.eye(Dim),
                                     size=N+1)     
        
        for i in range(N):
            [x[i+1],y[i+1]] = A .dot(np.asarray([x[i],y[i]])) + np.asarray([Nx[i+1],Ny[i+1]])
        
        
        
        dataset = [x,y]
    elif GenModel == 'Markov_Gaussian_1':
        (A,B,sigma_x, sigma_y, sigma_z) = Params
        Nx = np.random.multivariate_normal(mean=[0]*Dim,
                                     cov=sigma_x**2*np.eye(Dim),
                                     size=N+1) 

        Ny = np.random.multivariate_normal(mean=[0]*Dim,
                                     cov=sigma_y**2*np.eye(Dim),
                                     size=N+1)     

        Nz = np.random.multivariate_normal(mean=[0]*Dim,
                                     cov=sigma_z**2*np.eye(Dim),
                                     size=N+1) 
        x = np.zeros((N,Dim))
        y = np.zeros((N,Dim))
        z = np.zeros((N,Dim))
        ...
        for i in range(N-1):
            [x[i+1],y[i+1],z[i+1]] = np.linalg.inv(np.eye(3)-A).dot(B.dot(np.asarray([x[i],y[i],z[i]])) +np.asarray([Nx[i+1],Ny[i+1],Nz[i+1]]))
        dataset = [x,y,z]
    elif GenModel == 'Markov_Gaussian_2':
        (A,B,sigma_x, sigma_y, sigma_z) = Params
        N+=2
        Nx = np.random.multivariate_normal(mean=[0]*Dim,
                                     cov=sigma_x**2*np.eye(Dim),
        ...
        dataset = [x[2:N,:],y[2:N,:],z[2:N,:],x_,y_,z_,x__,y__,z__]

    elif GenModel == 'Markov_Gaussian_temp':
        (a_1, a_2, sigma_x, sigma_y, sigma_z) = Params
        ...
        y[1:N+2] = a_1*x[0:N+1] + np.random.multivariate_normal(mean=[0]*Dim,
                                     cov=sigma_y**2*np.eye(Dim),
                                     size=N+1)     

        z[1:N+2] = a_2*y[0:N+1] + np.random.multivariate_normal(mean=[0]*Dim,
                                     cov=sigma_z**2*np.eye(Dim),
                                     size=N+1) 
        ...
        dataset = [x,y,z,x_,y_,z_,x__,y__,z__]
